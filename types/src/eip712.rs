//! This implementation adapted from [`ethers-core`](https://github.com/gakonst/ethers-rs/blob/64b7f1ef1ac71fefcacc44d8ff1ddfcb8e6b5417/ethers-core/src/types/transaction/eip712.rs)
...
impl TryFrom<StringifiedNumeric> for Uint {
    type Error = String;

    fn try_from(value: StringifiedNumeric) -> Result<Self, Self::Error> {
        match value {
            StringifiedNumeric::U256(n) => Ok(n),
            StringifiedNumeric::Num(n) => Ok(Uint::from(n)),
            StringifiedNumeric::String(s) => {
                if let Ok(val) = s.parse::<u128>() {
                    Ok(val.into())
                } else if s.starts_with("0x") {
                    Uint::from_str(&s).map_err(|err| err.to_string())
                } else {
                    Uint::from_dec_str(&s).map_err(|err| err.to_string())
                }
            }
        }
    }
...
impl EIP712Domain {
    ...
    pub fn separator(&self) -> [u8; 32] {
        ...
        if let Some(ref name) = self.name {
            ty += "string name";
            tokens.push(Token::Uint(Uint::from(keccak256(name))));
            needs_comma = true;
        }
        ...
        if let Some(ref version) = self.version {
            ...
            ty += "string version";
            tokens.push(Token::Uint(Uint::from(keccak256(version))));
            needs_comma = true;
        }
        ...
        if let Some(salt) = self.salt {
            ...
            ty += "bytes32 salt";
            tokens.push(Token::Uint(Uint::from(salt)));
        }

        ty.push(')');

        tokens.insert(0, Token::Uint(Uint::from(keccak256(ty))));
...
pub fn encode_data(
    ...
    let mut tokens = vec![Token::Uint(Uint::from(hash))];
    ...
    if let Some(fields) = types.get(primary_type) {
        for field in fields {
            ...
            if let Some(value) = data.get(&field.name) {
                ...
                tokens.push(Token::Uint(Uint::zero()));
            } else {
...
fn parse_field_type(source: &str) -> Result<ParamType, Eip712Error> {
    match source {
        "bytes1" => Ok(ParamType::FixedBytes(1)),
        "bytes32" => Ok(ParamType::Uint(256)),
        ...
        _ => Err(Eip712Error::Message(format!("Unsupport types: {}", source))),
    }
}
...
mod tests {
    ...
    #[test]
    fn test_stringified_numeric() {
        let json_data = json!([1, "0x1"]);

        let _array: Vec<StringifiedNumeric> =
            serde_json::from_value(json_data).expect("Parse stringified numeric");
    }
