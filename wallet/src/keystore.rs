...
use crate::{hash::pbkdf2::pbkdf2_hmac, wallet::KeyProvider, WalletError};
...
pub trait KeyStoreEncrypt {
        ...
        B: KeyProvider,
    {
        ...
        let sec_key = SecretKey::from_be_bytes(&ciphertext)?;

        let address = Address::try_from(sec_key).ok();
...
mod tests {
    ...
    fn test_decrypt_ethersjs_keystore() {
        ...
        let pk = SecretKey::from_be_bytes(
            &keystore
                .decrypt_into("test")
                .expect("Decrypt keystore with password"),
        )
        ...
        let address = Address::try_from(pk).expect("Get address from descrypt pk");
