...
use crate::hash::pbkdf2::pbkdf2_hmac_array;
...
impl DriveKey {
    ...
    pub(crate) fn new_master_key(seed: [u8; 64]) -> Self {
        ...
        let private_key = SecretKey::from_be_bytes(key_data).expect("Create ecdsa signing key");
    ...
    pub(crate) fn child_key(&self, node: Bip44Node) -> Self {
        ...
        match node {
            Bip44Node::Hardened(_) => {
                ...
                h.write(&self.private_key.to_be_bytes())
                    .expect("Hmac write");
            }
            Bip44Node::Normal(_) => {
        ...
        let key = SecretKey::from_be_bytes(key_data).expect("Create ecdsa signing key");
        ...
        let private_key =
            SecretKey::from_be_bytes(&scalar.to_bytes()).expect("Create ecdsa signing key");
...
mod tests {
    ...
    fn test_gen_seed() {
        ...
        assert_eq!("0x15cba277c500b4e0c777d563278130f4c24b52532b3c8c45e051d417bebc5c007243c07d2e341a2d7c17bbd3880b968ca60869edab8f015be30674ad4d3d260f",seed.to_eth_hex());
    }
    ...
    fn test_hardhat_default_accounts() {
        ...
        fn check_drive(drive_key: &DriveKey, id: usize, pk: &str, expect_address: &str) {
            ...
            assert_eq!(key.private_key.to_be_bytes().to_eth_hex(), pk);
