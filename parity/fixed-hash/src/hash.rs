...
macro_rules! construct_fixed_hash {
	( $(#[$attr:meta])* $visibility:vis struct $name:ident ( $n_bytes:expr ); ) => {
		...
		impl<'a> TryFrom<&'a str> for $name {
            type Error = $crate::rustc_hex::FromHexError;
            ...
            fn try_from(value: &'a str) -> Result<Self, Self::Error> {
                ...
                for byte in result.as_mut() {
                    *byte = iter.next().ok_or(Self::Error::InvalidHexLength)??;
                }
                if iter.next().is_some() {
                    return Err(Self::Error::InvalidHexLength);
                }
                Ok(result)
            }
        }
    ...
    ( $name:ident ) => {
        impl $crate::quickcheck::Arbitrary for $name {
            fn arbitrary(g: &mut $crate::quickcheck::Gen) -> Self {
                let res: [u8; Self::len_bytes()] =
                    $crate::core_::array::from_fn(|_| u8::arbitrary(g));
                Self::from(res)
            }
        }
    };
